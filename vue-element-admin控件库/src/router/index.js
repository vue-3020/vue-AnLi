...
/**
* hidden: true                   如果“隐藏：真”将不显示在侧边栏（默认为false）
* alwaysShow: true               如果设置为true，将始终显示根菜单，无论其子路径长度
*                                如果没有设置总是显示，只有一个以上的路线下的儿童
*                                it will becomes nested mode, otherwise not show the root menu
* redirect: noredirect           如果“重定向：NoeNeCube”将不会在面包屑中重新分配
* name:'router-name'             名称使用<保持活着>（必须设置）！！！！）
* meta : {
    roles: ['admin','editor']     将控制页面角色（可以设置多个角色）
    title: 'title'               在子菜单和面包屑中显示的名称（推荐集）
    icon: 'svg-name'             图标显示在边栏中
    noCache: true                如果为真，则页面将不会被缓存（默认为false）
  }
**/
